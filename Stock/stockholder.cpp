...
StockHolder::StockHolder()
    : m_uBuyIntention(127)
    , m_uSaleIntention(127)
    , m_uId(++uStockHolderCount)
{
    StockClear::GetInstance().AddPosition(Position{ m_uId, 1000, 100000 });
}

void StockHolder::OnTime()
{
    ...
    if (uIntention < m_uBuyIntention)
    {
        ...
        if (uPrice > 0)
        {
            uint64_t uAmount = StockClear::GetInstance().CalcAmount(StockClear::GetInstance().GetPosition(m_uId).m_uBalance, uPrice);
            if (uAmount > 0)
            {
                std::uniform_int_distribution<uint64_t> timeStampRandom;
                uint64_t uTimeStamp = timeStampRandom(m_randomEngine);
                StockExchange::GetInstance().OnEntrust(Entrust{ uPrice, uAmount, uTimeStamp, m_uId, Direction::BUY });
            }
        }
    }
    else if ((255 - uIntention) < m_uSaleIntention)
    {
        ...
        if (uPrice > 0)
        {
            uint64_t uAmount = StockClear::GetInstance().GetPosition(m_uId).m_uAmount;
            if (uAmount > 0)
            {
                std::uniform_int_distribution<uint64_t> timeStampRandom;
                uint64_t uTimeStamp = timeStampRandom(m_randomEngine);
                StockExchange::GetInstance().OnEntrust(Entrust{ uPrice, uAmount, uTimeStamp, m_uId, Direction::SALE });
            }
        }
    }
}